...
export default class NotificationScheduleService extends Service {
  ...
  /** 启动时注册所有 enabled schedule 到 BullMQ */
  async registerAll(): Promise<number> {
    const list = await this.ctx.model.NotificationSchedule.findAll({ where: { enabled: 1 } } as any);
    let count = 0;
    for (const s of list) {
      const ss = s as any;
      if (!HANDLERS[ss.handler]) {
        this.ctx.logger.warn(`[schedule] handler '${ss.handler}' missing for ${ss.code}`);
        continue;
      }
      try {
        ...
        this.ctx.logger.error(`[schedule] failed to register ${ss.code}: ${e.message}`);
      }
    }
    return count;
  }
  ...
  async executeSchedule(scheduleId: number) {
    ...
    try {
      ...
      await ss.update({
        ...
        nextFireAt: nextFireFromCron(ss.cronExpr),
      });
      return r;
    } catch (e: any) {
      const { nextFireFromCron } = require('../lib/cronHelper');
