# P1-04：DB 迁移 018（Task 4）

> 子文件 4/12，对应 [P1 总览](./2026-05-16-notification-phase-1-00-overview.md) Task 4。

**Goal:** 创建 11 张通知系统表 + 21 条预置类型 + 6 条预置频控 + 3 条预置渠道配置 + 14 个权限码 + 4 角色绑定。提供回滚 SQL。

**Files:**
- Create: `super-tool-node/database/018_add_notification_system.sql`
- Create: `super-tool-node/database/018_rollback.sql`

**前置依赖**：[Task 1](./p1-01-deps-config.md)（仅依赖配置）

---

## Step 1: 编写主迁移文件

- [ ] 创建 `super-tool-node/database/018_add_notification_system.sql`
...
> **重要**：确认 `roles` 表中实际存在 `super_admin`/`admin`/`operator`/`data_analyst` 这 4 个角色（参考迁移 014b/006/011 等）。如有出入需调整角色 code。

---

## Step 2: 编写回滚 SQL

- [ ] 创建 `super-tool-node/database/018_rollback.sql`：
...
```bash
cd super-tool-node
node scripts/db-cli.js migrate database/018_add_notification_system.sql
```

或手动执行：

```bash
mysql -u <user> -p <database> < super-tool-node/database/018_add_notification_system.sql
```

Expected: 无错误输出，所有 SQL 语句执行成功。

**故障排查**：
- 若 `roles` 表查不到 `super_admin` 等：先确认角色 code，调整 SQL 后重跑（INSERT IGNORE 风格的 NOT EXISTS 已确保幂等）
- 若 `user_profiles` 表不存在 `IF NOT EXISTS` 关键字（旧版 MySQL）：去掉 `IF NOT EXISTS`，但需先手动判断字段是否已存在
...
SELECT COUNT(*) AS table_count FROM information_schema.tables 
  WHERE table_schema = '<database>' AND table_name LIKE 'notification_%';
SELECT COUNT(*) AS type_count FROM notification_types;
SELECT COUNT(*) AS rate_count FROM notification_rate_limit_config;
SELECT COUNT(*) AS channel_count FROM notification_channel_configs;
SELECT COUNT(*) AS perm_count FROM permissions WHERE code LIKE 'notification:%';
SELECT r.code AS role, COUNT(rp.id) AS perm_count
  FROM roles r LEFT JOIN role_permissions rp ON r.id = rp.role_id
  LEFT JOIN permissions p ON rp.permission_id = p.id AND p.code LIKE 'notification:%'
  WHERE r.code IN ('super_admin','admin','operator','data_analyst')
  GROUP BY r.code;
"
```
...
- channel_count = 3
- perm_count = 14
- super_admin 14, admin 13, operator 10, data_analyst 7
...
```bash
git add super-tool-node/database/018_add_notification_system.sql super-tool-node/database/018_rollback.sql
git commit -m "feat(notification): add migration 018 with 11 tables, 21 preset types and 14 permissions

- 11 tables: types/templates/template_versions/audiences/tasks/messages/preferences/quiet_hours/rate_limit/channels/send_logs
- 21 preset notification_types (5 system + 11 business + 2 marketing + 4 verify_code), all is_system=1
- 6 preset rate limit rules (P1 not enforced, P2 will activate)
- 3 preset channel configs (in_app native, email smtp placeholder, sms mock)
- 14 new permissions (notification:type:view ~ notification:config:manage)
- Role bindings: super_admin 14, admin 13, operator 10, data_analyst 7
- ALTER user_profiles ADD notification_global_enabled
...
- [ ] `database/018_add_notification_system.sql` 已创建
- [ ] `database/018_rollback.sql` 已创建
- [ ] 11 张 `notification_*` 表存在
- [ ] `notification_types` 21 条
- [ ] `notification_rate_limit_config` 6 条
- [ ] `notification_channel_configs` 3 条
- [ ] `permissions` 新增 14 条
- [ ] `role_permissions` 4 个角色绑定数量符合预期
- [ ] `user_profiles.notification_global_enabled` 字段存在
- [ ] git commit 已提交

完成本 Task 后请进入 [`p1-05-models.md`](./p1-05-models.md)。
